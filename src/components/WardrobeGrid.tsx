...
export default function WardrobeGrid({ items, onDelete, onClickItem }: WardrobeGridProps) {
          ...
          <AnimatePresence mode="popLayout">
            {filteredItems.map((item) => (
              <motion.div
                ...
                className="group relative bg-white border border-gray-100 rounded-2xl p-4 shadow-sm hover:shadow-md transition-all cursor-pointer"
              ...
              >
                ...
                <div className="aspect-[3/4] bg-gray-50 rounded-xl mb-3 flex items-center justify-center overflow-hidden relative">
                  ...
                  {/* AI rozeti */}
                  {(item as any).aiAnalyzed && (
                    <div className="absolute top-2 left-2 bg-indigo-600 text-white text-[9px] font-bold px-1.5 py-0.5 rounded-md">
                      AI
                    </div>
                  )}

                  {/* Eksik bilgi uyarısı */}
                  {(!item.color || !item.style || !item.subCategory) && !(item as any).aiAnalyzed && (
                    <div className="absolute top-2 left-2 bg-amber-400 text-white text-[9px] font-bold px-1.5 py-0.5 rounded-md" title="Eksik bilgi: renk / stil / alt kategori">
                      ?
                    </div>
                  )}
