...
async function startServer() {
  ...
  // ─── Güvenlik (CORS) ─────────────────────────────────────────────────────
  // Vercel'de yayınlanınca sadece kendi domaininize izin vermek için:
  const allowedOrigins = process.env.NODE_ENV === 'production' 
    ? [process.env.APP_URL, 'https://samethabali.github.io'] // Kendi domainlerinizi buraya ekleyin
  ...
  app.use(express.json({ limit: '10mb' })); // Güvenlik: Yükü 20mb'dan 10mb'a çektik
  ...
  app.post('/api/wardrobe/upload', upload.single('image'), async (req, res) => {
    try {
      ...
      const newItem = new ItemModel({
        ...
        name:         itemData.name        || analysis?.name        || req.file.originalname,
